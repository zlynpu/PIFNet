...
class ResUNet2(ME.MinkowskiNetwork):
  ...
  def local_fusion(self, coord, feature, image_feature, image_shape, extrinsic, intrinsic, id, voxel_size=0.3):
    ...
    for length in lengths:
      ...
      point_in_camera = extrinsic[batch_id, :, :]@point_in_lidar
      point_in_image = intrinsic[batch_id, :, :]@point_in_camera
      point_z = point_in_image[-1,:]
      point_in_image = point_in_image/point_z
      ...
      img_feature = grid_sample(feature_map, xy, mode='bilinear', padding_mode='zeros', align_corners=False)
      img_feature = img_feature.squeeze(2)
      img_feature = img_feature.transpose(1,2)
      img_feature = img_feature.squeeze(0)
      img_feature[point_z<=0] = 0
